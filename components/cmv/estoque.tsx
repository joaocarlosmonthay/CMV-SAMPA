...
import { Package, ShoppingCart, DollarSign, Trash2, Save, CheckCircle2, ShieldCheck, ArrowDownToLine } from "lucide-react"
...
export function Estoque({ dataInicio, dataFim, produtos, data, contagemInicial, contagemFinal, onChange, onSemanaFechada, isReadOnly = false }: EstoqueProps) {
  ...
  const handleSalvarCompra = async () => {
    ...
    // A MÁGICA ACONTECE AQUI: Pegamos o Total da Nota e dividimos pela Qtd para salvar o Unitário no Banco perfeitamente!
  ...
  const handleSalvarContagem = async () => {
    ...
    const inserts = Object.entries(contagem).map(([prodId, dados]) => ({
      ...
      valor_unitario: parseFloat(dados.valor.replace(',', '.')), // Aqui é Custo Unitário mesmo
      tipo_contagem: tipo,
  ...
  const handlePuxarAnterior = async () => {
    ...
    if (estAnterior && estAnterior.length > 0) {
        ...
        toast.success("Estoque importado! Revise os valores e clique em Salvar.")
    } else {
  ...
  return (
    <div className="space-y-6 relative">
      ...
      {isReadOnly && (
        <div className="bg-red-50 border border-red-200 text-red-600 p-4 rounded-2xl flex items-center gap-3 font-bold shadow-sm">
          <ShieldCheck className="w-5 h-5"/> Você está consultando uma semana fechada. Edições foram desativadas por segurança.
        </div>
      )}
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-[10px] uppercase font-black text-slate-400 border-b border-slate-100">
                <tr>
                  ...
                  {!isReadOnly && <th className="pb-3 px-4 text-right">Ação</th>}
                </tr>
              </thead>
        ...
        <div className="bg-white p-8 rounded-[32px] shadow-sm border space-y-6">
          <div className="flex flex-col md:flex-row justify-between md:items-center bg-slate-50 p-6 rounded-2xl border border-slate-200 gap-4">
             ...
             <div className="flex gap-3">
               ...
               {!isReadOnly && (
                 <button onClick={handleSalvarContagem} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-700 flex items-center gap-2 shadow-sm"><Save className="w-5 h-5"/> Salvar Estoque</button>
               )}
             </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {produtos.map(p => (
              <div key={p.id} className={`p-4 border border-slate-200 bg-white rounded-2xl flex flex-col gap-3 shadow-sm ${isReadOnly ? 'opacity-70' : 'hover:border-blue-300 transition-colors'}`}>
                ...
                <div className="flex gap-2">
                    <div className="flex-1">
                      ...
                      <input type="text" disabled={isReadOnly} className="w-full p-2.5 border border-slate-200 rounded-lg font-bold text-slate-700 outline-none focus:border-blue-500 bg-slate-50 focus:bg-white disabled:bg-slate-100 disabled:cursor-not-allowed" value={contagem[p.id]?.qtd || ""} onChange={e => setContagem({...contagem, [p.id]: { ...contagem[p.id], qtd: e.target.value }})} />
                    </div>
                    <div className="flex-1">
                      <label className="text-[10px] font-black text-slate-400 uppercase">Custo Unitário</label>
                      <input type="text" disabled={isReadOnly} className="w-full p-2.5 border border-slate-200 rounded-lg font-bold text-slate-700 outline-none focus:border-blue-500 bg-slate-50 focus:bg-white disabled:bg-slate-100 disabled:cursor-not-allowed" value={contagem[p.id]?.valor || ""} onChange={e => setContagem({...contagem, [p.id]: { ...contagem[p.id], valor: e.target.value }})} />
                    </div>
                </div>
              </div>
            ))}
      ...
      {aba === "faturamento" && (
        <div className="bg-white p-12 rounded-[40px] shadow-sm border flex flex-col items-center justify-center space-y-8 max-w-2xl mx-auto mt-4 relative">
           ...
           <div className="w-full max-w-sm relative">
             ...
             <input type="text" disabled={isReadOnly} value={faturamento} onChange={e => setFaturamento(e.target.value)} className="w-full pl-16 pr-6 py-5 text-4xl font-black text-slate-800 bg-slate-50 border-2 border-slate-200 rounded-3xl outline-none focus:border-emerald-500 disabled:bg-slate-100 disabled:cursor-not-allowed text-center" placeholder="0.00" />
           </div>
